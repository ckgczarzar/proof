...
from concept_proof_process import MyProcess
...
class bThread(threading.Thread):
	...
	def run(self):
		...
		while not self.stoprequest.isSet():
			...
			#result = os.system("ping %s -c %d -w 1" % (ip, times))
			#cmd = "ping %s -c %s -w 5" % (ip, times)
			#process = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
			#result = process.communicate()[0]
			#logger.debug("self.work_info: %s" % self.work_info)
			#self.result_q.put((self.work_info))
...
def shorewall_recover_system():
	shorewall_status_string = subprocess.Popen(["shorewall", "status"], stdout=subprocess.PIPE).communicate()[0]
	if shorewall_status_string.find("is stopped") == -1:
		# shorewall UP
		#logger.debug("Shorewall start OK, save the config to /var/lib/shorewall/restore")
		subprocess.call("shorewall save", shell=True)
	else:
		# shorewall DOWN
		#logger.info("Shorewall start failed, restore the config from /var/lib/shorewall/restore")
...
def inqueue(object):
	global maximun_record
	global queue 
	if len(queue) < maximun_record:
		queue.append(object)
	else:
		queue.pop(0)
		queue.append(object)
...
class MyTest(unittest.TestCase):
	...
	def test_json(self):
		snmp = Snmpd.model_exit()
		print snmp
		self.assertEqual(snmp,"model")

	#def test_fib(self):
	#	self.assertDictContainsSubset({"name": "John"}, {"name": "John", "age": 21})
